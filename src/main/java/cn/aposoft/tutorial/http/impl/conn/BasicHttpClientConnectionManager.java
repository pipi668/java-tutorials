...
package cn.aposoft.tutorial.http.impl.conn;
 ...
 * thread-safe it ought to be used by one execution thread only, as only one
 * thread a time can lease the connection at a time.
 * <p>
 * This connection manager will make an effort to reuse the connection for
 * subsequent requests with the same {@link HttpRoute route}. It will, however,
 * close the existing connection and open it for the given route, if the route
 * of the persistent connection does not match that of the connection request.
 * If the connection has been already been allocated
...
public class BasicHttpClientConnectionManager implements HttpClientConnectionManager, Closeable {
    ...
    public BasicHttpClientConnectionManager(final Lookup<ConnectionSocketFactory> socketFactoryRegistry,
            final HttpConnectionFactory<HttpRoute, ManagedHttpClientConnection> connFactory, final SchemePortResolver schemePortResolver,
    ...
    protected void finalize() throws Throwable {
        try {
            shutdown();
        } finally { // Make sure we call overridden method even if shutdown
                    // barfs
            super.finalize();
        }
    }
